    ...
    <div className="min-h-screen bg-white dark:bg-slate-950">
      ...
      <div className="mx-auto max-w-[1400px] px-4 md:px-6 py-4 md:py-6 flex gap-6">
            ...
            {currentTab?.sections.map(s => (
              <div key={displayLabel(s.section)} className="space-y-1">
                <button
                  ...
                  onClick={() => {
                    const el = document.getElementById(`sec-${displayLabel(s.section)}`)
                  ...
                  {s.categories.map(c => (
                    <li key={`${displayLabel(s.section)}-${displayLabel(c.category)}`}>
                      <button
                        ...
                        onClick={() => {
                          const el = document.getElementById(`cat-${displayLabel(s.section)}::${displayLabel(c.category)}`)
                          if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' })
                        }}
                      >
          ...
          {!isSearching && showFav && (
            <>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                {allPrompts.filter(p => isFavorite(p.id)).map(p => (
                  <PromptCard
                    key={p.id}
                    id={p.id}
                    text={p.text}
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleExportCustom} className="px-3 py-1.5 rounded-2xl bg-amber-500 text-white hover:bg-amber-400 dark:hover:bg-amber-400">
            Eksportér
          </button>
          <button onClick={handleImportClick} className="px-3 py-1.5 rounded-2xl bg-amber-500 text-white hover:bg-amber-400 dark:hover:bg-amber-400">
            Importér
          </button>
